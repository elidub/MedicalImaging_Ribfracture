...
class RetinaNetLoss(nn.Module):
    ...
    def forward(self, pred_box, pred_cls, gt_box, gt_cls):
        ...
        gt_cls_oh = one_hot(gt_cls.squeeze(-1).long(), self.num_classes).float()
        ...
        p = pred_cls.sigmoid()
        ce = cross_entropy(pred_cls, gt_cls_oh)
        ...
        box_loss = torch.nan_to_num(box_loss.sum(dim=-1) / pos_mask_norm)
        cls_loss = torch.nan_to_num(cls_loss.sum(dim=-1) / pos_mask_norm)
